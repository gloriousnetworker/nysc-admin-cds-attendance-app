...
export default function LGIDashboardSidebar({ activeSection, setActiveSection, darkMode }) {
  ...
  const menuItems = [
    ...
    { id: 'corpers', label: 'Corpers', icon: '🎓', color: 'bg-gradient-to-r from-green-500 to-emerald-500' },
    { id: 'ppa', label: 'PPA Management', icon: '🏢', color: 'bg-gradient-to-r from-purple-500 to-pink-500' },
    { id: 'cds', label: 'CDS Groups', icon: '👥', color: 'bg-gradient-to-r from-yellow-500 to-amber-500' },
    { id: 'attendance', label: 'Attendance', icon: '📝', color: 'bg-gradient-to-r from-indigo-500 to-blue-500' },
    { id: 'verification', label: 'Verifications', icon: '✅', color: 'bg-gradient-to-r from-pink-500 to-rose-500' },
    { id: 'reports', label: 'Reports', icon: '📈', color: 'bg-gradient-to-r from-teal-500 to-green-500' },
    { id: 'complaints', label: 'Complaints', icon: '⚠️', color: 'bg-gradient-to-r from-orange-500 to-red-500' },
    { id: 'profile', label: 'Profile', icon: '👤', color: 'bg-gradient-to-r from-gray-500 to-gray-700' },
    { id: 'settings', label: 'Settings', icon: '⚙️', color: 'bg-gradient-to-r from-red-500 to-pink-500' }
  ];
    ...
    <aside className={`hidden md:block ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg ${collapsed ? 'w-16' : 'w-64'} transition-all duration-300 h-[calc(100vh-4rem)] sticky top-16`}>
      ...
      {!collapsed && (
        <div className={`p-4 border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'} mt-4`}>
          <div className={`text-xs font-semibold mb-2 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>FCT JURISDICTION STATS</div>
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <span className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>FCT Corpers</span>
              <span className="font-bold text-[#008753] dark:text-green-400">285</span>
            </div>
            <div className="flex justify-between items-center">
              <span className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Active PPAs</span>
              <span className="font-bold text-blue-600 dark:text-blue-400">68</span>
            </div>
            <div className="flex justify-between items-center">
              <span className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>CDS Groups</span>
