...
export default function PPADashboardSidebar({ activeSection, setActiveSection, darkMode }) {
  ...
  const menuItems = [
    ...
    { id: 'corpers', label: 'My Corpers', icon: '🎓', color: 'bg-gradient-to-r from-green-500 to-emerald-500' },
    { id: 'attendance', label: 'Attendance', icon: '📝', color: 'bg-gradient-to-r from-purple-500 to-pink-500' },
    { id: 'schedule', label: 'Work Schedule', icon: '📅', color: 'bg-gradient-to-r from-yellow-500 to-amber-500' },
    { id: 'evaluation', label: 'Evaluations', icon: '⭐', color: 'bg-gradient-to-r from-indigo-500 to-blue-500' },
    { id: 'reports', label: 'Reports', icon: '📈', color: 'bg-gradient-to-r from-pink-500 to-rose-500' },
    { id: 'profile', label: 'Organization', icon: '🏢', color: 'bg-gradient-to-r from-teal-500 to-green-500' },
    ...
    <aside className={`hidden md:block ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg ${collapsed ? 'w-16' : 'w-64'} transition-all duration-300 h-[calc(100vh-4rem)] sticky top-16`}>
      ...
      {!collapsed && (
        <div className={`p-4 border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'} mt-4`}>
          <div className={`text-xs font-semibold mb-2 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>FCT ORGANIZATION STATS</div>
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <span className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Active Corpers</span>
              <span className="font-bold text-[#008753] dark:text-green-400">10</span>
            </div>
            <div className="flex justify-between items-center">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold text-blue-600 dark:text-blue-400">4.9</span>
            </div>
