    ...
    <aside className={`hidden md:block ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg ${collapsed ? 'w-16' : 'w-64'} transition-all duration-300 h-[calc(100vh-4rem)] sticky top-16 dark-mode-transition`}>
      ...
      {!collapsed && (
        <div className={`p-4 border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'} mt-4`}>
          <div className={`text-xs font-semibold mb-2 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>KOGI CDS STATS</div>
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <span className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Kogi Members</span>
              <span className="font-bold text-[#006600] dark:text-green-500">48</span>
            </div>
            <div className="flex justify-between items-center">
            ...
            <div className="flex justify-between items-center">
              <span className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Kogi Att. Rate</span>
              <span className="font-bold text-blue-600 dark:text-blue-400">88%</span>
            </div>
            <div className="flex justify-between items-center">
              <span className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Active in Lokoja</span>
              <span className="font-bold text-[#FF9900] dark:text-orange-400">48</span>
            </div>
            <button className="w-full mt-3 bg-gradient-to-r from-[#006600] to-[#008800] text-white py-2 px-4 rounded-lg hover:opacity-90 transition-colors duration-300 font-medium text-sm">
