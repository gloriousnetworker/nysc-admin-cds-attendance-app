...
export default function DashboardSidebar({ activeSection, setActiveSection, darkMode }) {
  ...
  const menuItems = [
    ...
    { id: 'ppa', label: 'PPA Owners', icon: '🏢', color: 'bg-gradient-to-r from-purple-600 to-purple-400' },
    { id: 'cds', label: 'CDS Coordinators', icon: '👥', color: 'bg-gradient-to-r from-yellow-600 to-yellow-400' },
    { id: 'lgi', label: 'LG Inspectors', icon: '🏛️', color: 'bg-gradient-to-r from-indigo-600 to-indigo-400' },
    { id: 'zi', label: 'Zonal Inspectors', icon: '🌍', color: 'bg-gradient-to-r from-pink-600 to-pink-400' },
    { id: 'admins', label: 'All Administrators', icon: '👑', color: 'bg-gradient-to-r from-red-600 to-red-400' },
    { id: 'attendance', label: 'Attendance', icon: '📝', color: 'bg-gradient-to-r from-teal-600 to-teal-400' },
    { id: 'reports', label: 'Reports', icon: '📈', color: 'bg-gradient-to-r from-orange-600 to-orange-400' },
    ...
    <aside className={`hidden md:block ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg ${collapsed ? 'w-16' : 'w-64'} transition-all duration-300 h-[calc(100vh-4rem)] sticky top-16`}>
      ...
      {!collapsed && (
        <div className={`p-4 border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'} mt-4`}>
          <div className={`text-xs font-semibold mb-2 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>LAGOS STATS</div>
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-700 dark:text-gray-300">Lagos Corpers</span>
              <span className="font-bold text-[#003366] dark:text-blue-400">2,156</span>
            </div>
            <div className="flex justify-between items-center">
            ...
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-700 dark:text-gray-300">LGAs Covered</span>
              <span className="font-bold text-purple-600 dark:text-purple-400">20</span>
            </div>
            <button className="w-full mt-3 bg-gradient-to-r from-[#003366] to-[#004080] text-white py-2 px-4 rounded-lg hover:opacity-90 transition font-medium text-sm">
